...
motif_matches_chip_only %>%
  ...
  ylab("fpQTL effect size  (relative to stronger motif)") +
  # xlim(c(-15.2, 15.1)) +
  ylim(c(-0.32, 0.2)) +
  stat_cor(method = "spearman", label.y = -0.23, label.x = 4, label.x.npc = 0.55, size = 4) +
  # ylim(c(NA, 0.15)) +
  theme_classic()

ggsave(sprintf("figures/%s/%s_concordance_scatter_%s.png", MOTIF_THRESHOLD, FP_METHOD, MOTIF_THRESHOLD), 
       width = 4, height = 4.4, units = "in")
```
...
binom.test(concordance[2], sum(concordance), p = 0.5)
...
concordant_fpQTLs %>%
  write.table(paste0("concordant_fpQTLs/", MOTIF_THRESHOLD, 
...
for (COUNT_CUTOFF in 2:4) {
  ...
  ggsave(paste0("figures/", MOTIF_THRESHOLD, "/", FP_METHOD,
                "_concordance_bar_plot_cutoff=", COUNT_CUTOFF, ".png"), width = 5, height = 4, units = "in")
}
...
ggsave(paste0("figures/", MOTIF_THRESHOLD, "/", FP_METHOD,
                "_concordance_bar_plot_cutoff=", COUNT_CUTOFF, "_grouped.png"), width = 4, height = 4, units = "in")
```
...
ggsave(paste0("figures/", MOTIF_THRESHOLD, "/fpQTL-motif_no_chip_odds_ratio.png"), width = 5, height = 7, units = "in")
```
...
# MANUSCRIPT - Supplementary Table 3
```{r}
motif_summary %>%
  ...
  write.table("motif_supplementary_table.txt",
              quote = FALSE, row.names = FALSE, sep = "\t")
```
